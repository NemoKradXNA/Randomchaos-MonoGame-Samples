    ...
    public class WorleyNoise : NoiseBase
    {
        ...
        public override float Sample2D(float x, float y)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                oyx = Mod((float)Math.Floor(px * K), 7.0f) * K - Ko;
                oyy = Mod((float)Math.Floor(py * K), 7.0f) * K - Ko;
                oyz = Mod((float)Math.Floor(pz * K), 7.0f) * K - Ko;
